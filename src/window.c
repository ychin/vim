...
win_split(int size, int flags)
{
	...
	if (!can_abandon(wp_curwin->w_buffer, cmdmod.cmod_curwin_force))
	{
	    no_write_message();
	    return FAIL;
	}
	...
	if (curwin != wp_curwin)
	{
	    ...
	    // win_split() does without cmdmod.cmod_curwin.
	    if (do_ecmd(fnum_curwin, NULL, NULL, NULL, ECMD_ONE,
			(cmdmod.cmod_curwin_force ? ECMD_FORCEIT : ECMD_HIDE), curwin) == FAIL)
	    {
		return FAIL;
	    }
	}
